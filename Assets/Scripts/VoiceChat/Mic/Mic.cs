    ...
    public class Mic : MonoBehaviour {
        ...
        void Update() {
            foreach (var pair in clips)
            {
                ...
                {
                    int lastLoopSampleLen = clip.samples - prevPos;
                    float[] lastLoopSamples = new float[lastLoopSampleLen - 1];
                    clip.GetData(lastLoopSamples, prevPos);
                    foreach (float sample in lastLoopSamples)
                        pcm.Enqueue(sample);

                    int currLoopSampleLen = pos;
                    if (currLoopSampleLen != 0)
                    {
                        float[] currLoopSamples = new float[currLoopSampleLen-1];
                        clip.GetData(currLoopSamples, 0);
                        foreach (float sample in currLoopSamples)
                            pcm.Enqueue(sample);
                    }
                }
